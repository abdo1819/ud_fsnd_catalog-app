...
catItem1 = CatItem(title="Hockey Stick", description="A long-handled stick with one curved end that is used in hockey."
                    , category=category1, user=user1)
...
catItem1 = CatItem(title="Corner flag", description="Each of the flags that denotes the corner of the field of play.", category=category1, user=user1)

session.add(catItem1)
session.commit()

catItem1 = CatItem(title="Bibs", description="bib for protecting players ", category=category1, user=user1)

session.add(catItem1)
session.commit()

catItem1 = CatItem(title="Hockey Kit bag", description="A duffel bag, duffle bag, or kit bag is a large cylindrical bag made of natural or synthetic fabric, historically with a top closure using a drawstring. Generally a duffel bag is used by non-commissioned personnel in the military, and for travel, sports and recreation by civilians. When used by sailors or marines a duffel is known as a \"seabag\". A duffel's open structure and lack of rigidity makes it adaptable to carrying sports gear and similar bulky objects"
                    , category=category1, user=user1)
